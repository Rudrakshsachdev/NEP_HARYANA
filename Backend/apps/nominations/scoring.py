...
def get_individual_indicator_scores(answers):
    ...
    # p6 (Max 6): ABC registered (Indicator 9 - Max 8)
    ans9 = answers.get('indicator_9', {})
    p6_val = 0
    if ans9.get('value') == 'Yes':
        try:
    ...
    scores['p8'] = int(round((p8_val * 4) / 6)) if p8_val else 0
    ...
    # p10 (Max 4): Alumni Connect (Indicator 15 - Max 6)
    ans15 = answers.get('indicator_15', {})
    p10_val = min(len(ans15.get('items', [])), 6) if ans15.get('value') == 'Yes' else 0
    scores['p10'] = int(round((p10_val * 4) / 6)) if p10_val else 0
    
    # p11 (Max 5): Gender Parity (Indicator 16 - Max 6)
    ans16 = answers.get('indicator_16', {})
    p11_val = min(len(ans16.get('items', [])), 6) if ans16.get('value') == 'Yes' else 0
    scores['p11'] = int(round((p11_val * 5) / 6)) if p11_val else 0
    
    # p12 (Max 5): Student Welfare guidelines (Indicator 18 - Max 6)
    ans18 = answers.get('indicator_18', {})
    p12_val = min(len(ans18.get('items', [])), 6) if ans18.get('value') == 'Yes' else 0
    scores['p12'] = int(round((p12_val * 5) / 6)) if p12_val else 0
    
    # p13 (Max 4): Online Courses / MOOCs (Indicator 19 - Max 4)
    ans19 = answers.get('indicator_19', {})
    scores['p13'] = 4 if ans19.get('value') == 'Yes' else 0
    
    # p14 (Max 4): MMTTC training (Indicator 20 - Max 6)
    ans20 = answers.get('indicator_20', {})
    p14_val = 0
    try:
        pct = float(ans20.get('percentage', 0))
    ...
    scores['p14'] = int(round((p14_val * 4) / 6)) if p14_val else 0
    
    # p15 (Max 8): Multidisciplinary Education -> Indicator 1 (Max 4)
    ans1 = answers.get('indicator_1', {})
    p15_val = 4 if ans1.get('value') == 'Yes' else 0
    scores['p15'] = int(round((p15_val * 8) / 4)) if p15_val else 0
    ...
    ans8 = answers.get('indicator_8', {})
    p17_val = 2 if ans8.get('value') == 'Yes' else 0
    scores['p17'] = int(round((p17_val * 4) / 2)) if p17_val else 0
    
    # p18 (Max 2): NIRF -> Indicator 10 (Max 4)
    ans10 = answers.get('indicator_10', {})
    p18_val = 4 if ans10.get('value') == 'Yes' else 0
    scores['p18'] = int(round((p18_val * 2) / 4)) if p18_val else 0
    
    # p19 (Max 6): OBE -> Indicator 13 (Max 4)
    ans13 = answers.get('indicator_13', {})
    p19_val = 4 if ans13.get('value') == 'Yes' else 0
    scores['p19'] = int(round((p19_val * 6) / 4)) if p19_val else 0
    
    # p20 (Max 2): Utilization of funds -> Indicator 17 (Max 6)
    ans17 = answers.get('indicator_17', {})
    p20_val = min(len(ans17.get('items', [])), 6) if ans17.get('value') == 'Yes' else 0
    scores['p20'] = int(round((p20_val * 2) / 6)) if p20_val else 0
